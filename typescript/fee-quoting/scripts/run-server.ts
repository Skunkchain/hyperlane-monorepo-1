...
async function main() {
  const args = await yargs(hideBin(process.argv))
    ...
    .option('warp-route-id', {
      alias: 'w',
      describe: 'Warp route ID from the registry',
      type: 'string',
      default: process.env.WARP_ROUTE_ID,
    })
    .option('registry', {
      alias: 'r',
  ...
  const config = ServerConfigSchema.parse({
    ...
    warpRouteId: args['warp-route-id'],
  ...
  rootLogger.info(
    `Starting fee quoting server for warp route ${config.warpRouteId}`,
  );
